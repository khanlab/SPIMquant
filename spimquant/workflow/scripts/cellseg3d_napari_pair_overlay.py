...
from np_algs import watershed, redistribute_weights, instance_to_binary, round_object_detection


PATH_PREFIX = snakemake.input.pred_result_dir
BLOCK_WIDTH = 4096  # take as much as possible
im = np.load(f'{PATH_PREFIX}/inference_im.npy')[:BLOCK_WIDTH, :BLOCK_WIDTH, :BLOCK_WIDTH]
seg = np.load(f'{PATH_PREFIX}/inference.npy')[:, :BLOCK_WIDTH, :BLOCK_WIDTH, :BLOCK_WIDTH]
