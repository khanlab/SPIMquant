...
import argparse
import shutil
from cvpl_tools.ome_zarr.io import write_da_as_ome_zarr_direct, write_da_as_ome_zarr
...
def get_ome_zarr() -> da.Array:
    if args.USE_SYNTHETIC_DATASET:
        ...
        zarr_group = zarr.open(args.ZARR_PATH, mode='w')
        write_da_as_ome_zarr_direct(zarr_group, da_arr=arr, MAX_LAYER=0)
...
def main():
    ...
    with dask.config.set({'temporary_directory': args.TMP_PATH}):
        ...
        write_da_as_ome_zarr(args.OUT_ZARR_PATH, tmp_path=args.TMP_PATH,
                             da_arr=copy_arr, lbl_arr=lbl_arr, lbl_name=args.LBL_NAME,
                             make_zip=True, MAX_LAYER=args.MAX_DOWNSAMPLING_LEVEL)
