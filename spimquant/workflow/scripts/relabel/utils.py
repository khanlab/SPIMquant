...
try:
    ...
    def find_contours(mask: ArrayLike) -> List[List[int]]:
        contour_coords, _ = cv2.findContours(
...
try:
    ...
    FeatureCollection = TypeVar("FeatureCollection", geojson.FeatureCollection, Any)

    def geojson_feature(coordinates_list: List[List[int]]) -> geojson.Feature:
        feature = geojson.Feature(geometry=geojson.Polygon([coordinates_list]))
        return feature

    def geojson_feature_collection(
        annotations_list: List[geojson.Feature],
    ...
    def geojson_feature_collection(annotations_list: List[dict]) -> dict:
        collection = {"features": annotations_list, "type": "FeatureCollection"}
        return collection


def get_valid_overlaps(
    chunk_location: List[int], num_chunks: List[int], ndim: int
) -> List[List[int]]:
    valid_indices = []

    for axis in range(ndim):
        for comb, k in itertools.product(
            itertools.combinations(range(ndim), axis), range(2 ** (ndim - axis))
        ):
            indices = list(
                np.unpackbits(
                    np.array([k], dtype=np.uint8), count=ndim - axis, bitorder="little"
                )
            )
            ...
            if all(
                map(
                    lambda level, coord, axis_chunks: (
                        level is None
...
def get_dest_selection(
    coord: int, axis_chunks: int, axis_overlap: int, axis_level: Union[int, None] = None
) -> slice:
    if axis_level is None:
        ...
        sel = slice(
            -axis_overlap * (2 if coord < axis_chunks - 1 else 1),
            -axis_overlap if coord < axis_chunks - 1 else None,
        )

    else:
        sel = slice(
            axis_overlap if coord > 0 else 0, axis_overlap * (2 if coord > 0 else 1)
        )
...
def get_source_selection(
    ...
    if axis_level is None:
        sel = slice(
            axis_overlap if coord > 0 else None,
            -axis_overlap if coord < axis_chunks - 1 else None,
        )
