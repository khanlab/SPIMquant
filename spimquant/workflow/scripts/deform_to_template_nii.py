...
if Path(in_zarr_url).suffix == ".zip":
    store = zarr.storage.ZipStore(
        Path(in_zarr_url).path, dimension_separator="/", mode="r"
    )
else:
    store = zarr.storage.FSStore(
        Path(in_zarr_url).path, fs=fs, dimension_separator="/", mode="r"
    )
...
flo_znimg = ZarrNii.from_ome_zarr(
    store, channels=[channel_index], **snakemake.params.flo_opts, **orient_opt
)
ref_znimg = ZarrNii.from_nifti(
...
deform_znimg = flo_ds_znimg.apply_transform(
