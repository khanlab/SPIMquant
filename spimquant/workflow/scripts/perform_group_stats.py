    ...
    for _, region in regions.iterrows():
        ...
        for metric in metrics:
            ...
            if len(g1_values) < 2 or len(g2_values) < 2:
                ...
                result[f"{metric}_mean_{group1_value}"] = g1_values.mean() if len(g1_values) > 0 else np.nan
                result[f"{metric}_mean_{group2_value}"] = g2_values.mean() if len(g2_values) > 0 else np.nan
            ...
            if pooled_std > 0:
                ...
                result[f"{metric}_cohens_d"] = cohens_d
            else:
                result[f"{metric}_cohens_d"] = np.nan
