...
def cellposeLabel(im_chunk, model=None):
    masks, _, _, _ = model.eval(
        im_chunk, diameter=64, channels=[0, 0], flow_threshold=0.4, do_3D=True
    )
...
img_da = (
    da.from_zarr(in_zarr, component=f"{level}")[channel_index, :, :, :]
    .squeeze()
    .rechunk(chunks=snakemake.params.chunks)
)
